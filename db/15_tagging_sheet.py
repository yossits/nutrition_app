...
The DSN is read from .env at the repository root — see db/_env.py.
  ...
  python db\\15_tagging_sheet.py --codes db\\block5_fat_codes.txt [--out PATH]

Output: --out, default db/tagging_sheet_report.txt (gitignored via
db/*_report.txt). Regenerated from scratch on every run.
...
def main():
    ...
    parser.add_argument("--out", default=str(HERE / "tagging_sheet_report.txt"))
    ...
    with conn_ctx as conn:
        ...
        # ---- the sheet --------------------------------------------------------
        out.write("TAGGING SHEET — block 5c, the block-4 fat list, with what the "
                  "code can propose and what the database already holds\n")
        ...
        out.write("\n\n" + RULE + "\n")
        out.write(f"▸ FAT — {len(all_items)} rows, grouped by §5.5 family\n")
        out.write(RULE + "\n")
    ...
    out.write("▸ COUNTS\n")
    out.write(RULE + "\n")
    out.write(f"  {len(codes)} codes · {len(codes) - len(missing)} in the 07 pool · "
